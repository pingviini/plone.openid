...
class TestOpenIdExtraction(unittest.TestCase):
    ...
    def testEmptyStringIdentityExtraction(self):
        """Test coverage for bug #7176. In the case where "" (i.e an empty
           ...
           would signify that an IOpenIdExtractionPlugin challenge was
           initialized.

           This test demonstrates our openid plugin's extractCredentials
           eliminates credentials that aren't in the openid.* namespace.
        """
        plugin = self.createPlugin()
        plugin.REQUEST.form.update(self.server_response)
        plugin.REQUEST.form["__ac_identity_url"] = ""
        creds = plugin.extractCredentials(plugin.REQUEST)
        self.failIf("__ac_identity_url" in creds)
    ...
    def testRedirect(self):
        ...
        self.assertRaises(Redirect, plugin.extractCredentials, plugin.REQUEST)
    ...
    def testUnallowedOpenIDProviderExtraction(self):
        ...
        plugin.REQUEST.form.update(self.server_response)
        plugin.REQUEST.form["__ac_identity_url"] = "http://wrong.myopenid.com"
        creds = plugin.extractCredentials(plugin.REQUEST)
        self.failIf("__ac_identity_url" in creds)
